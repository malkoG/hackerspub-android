...
**Arguments**: `onto upstream's main` (optional base branch, defaults to `main`)

## Instructions

1. Parse the base branch:
   - If `onto upstream's main` is provided, use it as the base branch
   - Otherwise, default to `main`

2. Gather information about the current branch:
   - Run `git branch --show-current` to get the current branch name
   - Run `git log <base>..HEAD --oneline` to see commits no...

### Prompt 81

Can we also flavoured app to be installed as separate app?
...
2. Gather information about the current branch:
   - Run `git branch --show-current` to get the current branch name
   - Run `git log <base>..HEAD --oneline` to see ...

### Prompt 83

For postcard, why it only shows image attachment only one?

### Prompt 84

Only show one image attachment.

### Prompt 85

Hmmmm... Still not. Also, Image should be distinguishhed by border
